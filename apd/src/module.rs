...
fn exec_install_script(module_file: &str, is_metamodule: bool, module_id: &str) -> Result<()> {
    ...
    // Only a metamodule that is actually doing the mounting gets to replace the
    // installer; in magic/disabled mode the built-in one is used.
...
fn _install_module(zip: &str) -> Result<()> {
    ...
    // Check if it's safe to install regular module. Only meaningful when the
    // metamodule owns mounting; magic mount handles these modules itself.
    if get_mount_mode() == defs::MOUNT_MODE_METAMODULE
        && !is_metamodule
        && needs_mount
