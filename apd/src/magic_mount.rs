...
use rustix::fs::{
    Gid, MetadataExt, Mode, MountPropagationFlags, Uid, UnmountFlags, bind_mount,
    chmod, chown, move_mount, unmount,
};
...
fn collect_module_files() -> Result<Option<Node>> {
    ...
    for entry in module_root.read_dir()?.flatten() {
        ...
        // 1. Collect $MODPATH/system
        let mod_system = entry.path().join("system");
        if mod_system.is_dir() {
            has_file |= system.collect_module_files(&mod_system)?;
        }
        
        // 2. Collect $MODPATH/$PARTITION
        for (partition, _) in partitions.iter() {
             let mod_part = entry.path().join(partition);
             if mod_part.is_dir() {
                    ...
                    match root.children.entry(name) {
                         ...
                         Entry::Occupied(mut o) => {
                             ...
                             for (child_name, child_node) in node.children {
                                 root_node.children.insert(child_name, child_node);
                             }
                             if node.replace {
