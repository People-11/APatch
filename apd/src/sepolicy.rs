...
impl PolicyStatement {
    fn parse(input: &str) -> IResult<&str, Self> {
        ...
        let (input, statement) = alt((
            map(NormalPerm::parse, |_| PolicyStatement::NormalPerm(())),
            map(XPerm::parse, |_| PolicyStatement::XPerm(())),
            map(TypeState::parse, |_| PolicyStatement::TypeState(())),
            map(Type::parse, |_| PolicyStatement::Type(())),
            map(TypeAttr::parse, |_| PolicyStatement::TypeAttr(())),
            map(Attr::parse, |_| PolicyStatement::Attr(())),
            map(TypeTransition::parse, |_| PolicyStatement::TypeTransition(())),
            map(TypeChange::parse, |_| PolicyStatement::TypeChange(())),
            map(GenFsCon::parse, |_| PolicyStatement::GenFsCon(())),
        ))
        .parse(input)?;
        let (input, _) = space0(input)?;
...
fn parse_sepolicy(input: &str, strict: bool) -> Result<Vec<PolicyStatement>> {
