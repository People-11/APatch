...
use crate::defs::MANAGER_PKG_FILE;
use crate::package::{get_package_changes, read_ap_package_config, synchronize_package_uid};
...
pub fn refresh_ap_package_list(skey: &CStr, mutex: &Arc<Mutex<()>>) {
    ...
    notify_package_changes();
}

/// Tell the manager which packages appeared or went away, so it can offer a
/// root decision for a new app. No-op until the manager registers itself.
fn notify_package_changes() {
    let Some(receiver) = read_receiver_target(MANAGER_PKG_FILE) else {
        return;
    };
    let manager_pkg = receiver.split('/').next().unwrap_or_default();

    let (installed, uninstalled) = get_package_changes();
    for pkg in installed {
        // The manager installing itself is not something to notify about.
        if pkg != manager_pkg {
            send_broadcast(&receiver, &pkg, true);
        }
    }
    for pkg in uninstalled {
        if pkg != manager_pkg {
            send_broadcast(&receiver, &pkg, false);
        }
    }
}

/// Reads the "<package>/<receiver class>" the manager wrote for us, accepting a
/// bare package name for compatibility.
fn read_receiver_target(path: &str) -> Option<String> {
    ...
    Some(if content.contains('/') {
        ...
        format!("{content}/{content}.InstallReceiver")
    })
}
...
fn send_broadcast(receiver: &str, pkg_name: &str, is_install: bool) {
    ...
    // Detached from apd's process group and cgroup: `am` must not be killed
    // along with apd, and must not inherit its cgroup limits.
    let spawned = unsafe {
    ...
    if let Err(e) = spawned {
        warn!("[send_broadcast] failed to notify about {pkg_name}: {e}");
    }
}
